...
int main(int argc, char const *argv[]) {
  ...
  for(i=0; i<nbTests; i++) {
    ...
    if(type == 0) {
      ...
      res_b32_check_v2.f32 = applyOp_float(op, res_b32_check_v2.f32, absErr_max_quarter);
      ...
      //  this should give a result that is within 2^{absErr_exp} of the result
      //  depending on the rounding mode used by vprec
      res_b64_check.f64 = applyOp_double(op, res_b64_check.f64, absErr_max_half);
      ...
      //add 2^{absErr_exp-2} to the result
      //  this should give a result that to the original result
      res_b64_check_v2.f64 = applyOp_double(op, res_b64_check_v2.f64, absErr_max_quarter);
