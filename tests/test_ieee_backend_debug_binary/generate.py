        ...
        if is_substring(hex_float, "nan"):
            ...
            if implicit_bit == "0":
                if args.normalize_denormal:
                    ...
                    self.mantissa = "{0:0<{size}b}".format(
                        int(self.mantissa, 2), size=self.mantissa_size
                    )
                ...
                if args.normalize_denormal:
                    ...
                    self.mantissa = self.mantissa.rjust(
                        len(self.mantissa) + offset, "0"
                    )
                    self.exponent = self.exponent_min
                    self.implicit_bit = "0"

            self.mantissa = remove_trailing_0(self.mantissa)
            self.string = "{sign}{i}.{mant} x 2^{exp}".format(
                sign=self.sign,
                i=self.implicit_bit,
                mant=self.mantissa,
                exp=self.exponent,
...
class Binary32(Binary):
    ...
    def __str__(self):
        ...
        exponent: {exponent}; \
        mantissa: {mantissa}]".format(
            sign=self.sign, exponent=self.exponent, mantissa=self.mantissa
        )
        return fmt
...
if "__main__" == __name__:
    parser = argparse.ArgumentParser(
        description="Generate output as --debug-binary mode of libinterflop_ieee.so"
    )
    parser.add_argument(
        "-t", "--float-type", choices=FLOAT_TYPES, required=True, help="float type"
    )
    parser.add_argument(
        "-f", "--filename", required=True, help="filename with values to print"
    )
    parser.add_argument(
        "--normalize-denormal",
        action="store_true",
        help="print denormals in a normalized way",
    )
