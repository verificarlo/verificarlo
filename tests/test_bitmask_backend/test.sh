...
for op in "+" "-" "*" "/" ; do
    ...
    verificarlo --function=operate --verbose -D REAL=float -D SAMPLES=$SAMPLES -D OPERATION="$op" -O0 test.c -o test
    check_status
    check 23 4 FLOAT RAND

    echo "Checking $op double"
    verificarlo --function=operate --verbose -D REAL=double -D SAMPLES=$SAMPLES -D OPERATION="$op" -O0 test.c -o test
    check_status
