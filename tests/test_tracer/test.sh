...
veritracer_analyzer() {
    FMT=${1}
    DIR=${2}
    echo "veritracer analyzer -f veritracer.dat --format=${FMT} --prefix-dir=${DIR} --output='${DIR}/veritracer' "
    veritracer analyzer --filename veritracer.dat --format=${FMT} --prefix-dir=${DIR} --output="${DIR}/veritracer"
    check_exit_code "Error with veritracer analyzer"
}
...
for FP_TYPE in "DOUBLE" "FLOAT"; do
	...
	for LANGUAGE in "C" "FORTRAN"; do 
	    ...
	    veritracer_analyzer $FMT $EXP_PATH
