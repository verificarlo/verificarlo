...
def shell(cmd, newEnv, maxTimeout=None):
    ...
    proc = subprocess.Popen(cmdNew.split(), stdout=subprocess.PIPE, env=env)
...
def getProfile(file, function_list):
    ...
    while i < len(Lines):
        ...
        if (
            len(function_list) != 0
            and (not Fname in function_list)
            and (not Fparent in function_list)
        ):
    ...
    FunctionsFrame = (
        ...
        .sort_values(["Ncalls", "ID"], ascending=False)
...
def save(Arguments, Operations, File):
    ...
    for i in Operations.index:
        f.write(
            "{ID}\t{Lib}\t{Int}\t{Float}\t{Double}\t{Prec64}\t{Range64}\t{Prec32}\t{Range32}\t{Ninputs}\t{Noutputs}\t{Ncalls}\n".format(
        ...
        j = arg_index
        while (
            j < arg_index + Operations.at[i, "Ninputs"] + Operations.at[i, "Noutputs"]
        ):
            f.write(
                "{IO}\t{ArgID}\t{Type}\t{Prec}\t{Range}\n".format(
...
def main():
    parser = argparse.ArgumentParser(
        description="The vfc_precexp script tries "
        "to minimize the precision for each function "
        "call of a code compiled with verificarlo"
    )
    parser.add_argument("runPath", help="command used to execute the target program")
    parser.add_argument(
        "cmpPath", help="command used to compare an execution with the reference"
    )

    parser.add_argument(
        "--maxTimeout", type=int, default=None, help="maximum execution time (ms)"
    )
    parser.add_argument(
        "-m",
        "--mode",
        choices=["arguments", "operations", "all"],
        default="all",
        help="function instrumentation mode",
    )
    parser.add_argument(
        "--vprec_mode",
        choices=["ieee", "ib", "ob", "all"],
        default="ob",
    ...
    set_environment_variable(
        "VFC_BACKENDS",
        "libinterflop_vprec.so --prec-output-file={}".format(vfc_profile_file),
        env,
    )
    ...
    if vfc_mode == "operations":
        ...
        set_environment_variable(
            "VFC_BACKENDS",
            "libinterflop_vprec.so --prec-input-file={} --daz --ftz --mode={} --instrument=operations".format(
                vfc_config_file, vfc_vprec_mode
            ),
            env,
        )
    ...
    if vfc_mode == "arguments":
        ...
        set_environment_variable(
            "VFC_BACKENDS",
            "libinterflop_vprec.so --prec-input-file={} --daz --ftz --mode={} --instrument=arguments".format(
                vfc_config_file, vfc_vprec_mode
            ),
            env,
        )
    ...
    if vfc_mode == "all":
        ...
        set_environment_variable(
            "VFC_BACKENDS",
            "libinterflop_vprec.so --prec-input-file={} --daz --ftz --mode={} --instrument=all".format(
                vfc_config_file, vfc_vprec_mode
            ),
            env,
        )
