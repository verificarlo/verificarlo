...
class InterflopTask:
    ...
    def cmpOneSample(self, i):
        rundir = self.nameDir(i)
        if self.subProcessRun[i] is not None:
    ...
    def sampleToComputeToGetFailure(self, nbRun):
        """Return the list of samples which have to be computed to perforn
           nbRun Success run : None mean Failure [] Mean Success 
        """
        listOfDir = [runDir for runDir in os.listdir(
            self.dirname) if runDir.startswith("dd.run")]
        done = []
        for runDir in listOfDir:
            status = int(
                (open(os.path.join(self.dirname, runDir, "returnVal")).readline()))
            if status != 0:
                return None
            done += [runDir]
...
class DDStoch(DD.DD):
    ...
    def mergeList(self):
        ...
        listOfExcludeFile = [x for x in os.listdir(
            dirname) if self.isFileValidToMerge(x)]
    ...
    def checkReference(self):
        ...
        if retval != 0:
            ...
            print("Suggestions:")
            print("\t1) check the correctness of the %s script" %
                  self.compare_)

            print("Files to analyze:")
            print("\t run output: " + os.path.join(self.ref_, "dd.out") +
                  " " + os.path.join(self.ref_, "dd.err"))
            print("\t cmp output: " + os.path.join(self.ref_, "checkRef.out") +
                  " " + os.path.join(self.ref_, "checkRef.err"))
            failure()
    ...
    def splitDeltas(self, deltas, nbRun, granularity):
        ...
        # result : set of smallest (each subset with repect with granularity lead to success)
        res = []
    ...
    # runTab=splitTab ,granularity=2):
    def SsplitDeltas(self, deltas, runTab, granularity):
        ...
        for run in runTab:
            ...
            flatNextCurrent = [
                flatItem for nextCurrentItem in nextCurrent for flatItem in nextCurrentItem]
            remainDeltas = [
                delta for delta in deltas if delta not in flatNextCurrent]

            # apply split to remainDeltas
            self.report_progress(remainDeltas, algo_name)
            nextCurrent.extend(self.splitDeltas(
                remainDeltas, run, granularity))

            currentSplit = nextCurrent

        return currentSplit

    # SrunTab=rddMinTab, dicRunTab=splitTab, granularity=2):
    def DRDDMin(self, deltas, SrunTab, dicRunTab, granularity):
        # name for progression
        algo_name = "DRDDMin"
    ...
    def deltaFailedMsg(self, delta):
        ...
        print("Suggestions:")
        print("\t1) check the correctness of the %s script : the failure criteria may be too large" %
              self.compare_)
    ...
    def allDeltaFailedMsg(self, deltas):
        ...
        print("\t2) check the correctness of the %s script : the failure criteria may be too large" %
              self.compare_)
        print(
            "\t3) set the env variable INTERFLOP_DD_UNSAFE : be careful it is realy unsafe")
    ...
    def _test(self, deltas, nbRun=None):
        ...
        if not os.path.exists(dirname):
            os.makedirs(dirname)
            self.genExcludeIncludeFile(
                dirname, deltas, include=True, exclude=True)

        vT = InterflopTask(dirname, self.ref_, self.run_, self.compare_,
                           nbRun, self.config_.get_maxNbPROC(), self.sampleRunEnv(dirname))
