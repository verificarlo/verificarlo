          ...
          if (isa<CallInst>(pi)) {
            ...
            if (Function *f = cast<CallInst>(pi)->getCalledFunction()) {
              ...
              if (MDNode *N = pi->getMetadata("dbg")) {
                ...
                std::string FunctionName = File + "/" + Name + "_" + Line +
                                           "_" + std::to_string(++inst_cpt);
                ...
                if (!(is_intrinsic && (!use_float && !use_double))) {
                  ...
                  Constant *haveFloat = ConstantInt::get(
                      Type::getInt8Ty(M.getContext()), use_float);
                  Constant *haveDouble = ConstantInt::get(
                      Type::getInt8Ty(M.getContext()), use_double);
