...
double FastAccSumOpt_unroll4_(double *p, unsigned int n) {
  double *q, r;
  
  q = (double *)malloc(sizeof(double)*n);
  memcpy(q, p, sizeof(double)*n);
  r = FastAccSumOpt_unroll4_In(q, n);
  free(q);
  return r;
}


double FastAccSumOpt_unroll4_In(double *p, unsigned int n) {
	...
	do
	{
		...
		for(i=0;i<n;i+=4)
		{
			...
			q=sigma-sigma_prime;
			p[i+1]=p[i+1]-q;
			sigma_prime=sigma+p[i+2];
			q=sigma_prime-sigma;
			p[i+2]=p[i+2]-q;
			sigma= sigma_prime+p[i+3];
			q=sigma-sigma_prime;
			p[i+3]=p[i+3]-q;
		}
		...
		t_prime=t+tau;
		if (t_prime==0) 
			return res=FastAccSumOpt_unroll4_In(p,n);
