...
double FastAccSumOpt_unroll3(double *p, unsigned int n) {
  double *q, r;
  
  q = (double *)malloc(sizeof(double)*n);
  memcpy(q, p, sizeof(double)*n);
  r = FastAccSumOpt_unroll3_In(q, n);
  free(q);
  return r;
}


double FastAccSumOpt_unroll3_In(double *p, unsigned int n) {
	...
	do
	{
		...
		t_prime=t+tau;
		if (t_prime==0) 
			return res=FastAccSumOpt_unroll3_In(p,n);
