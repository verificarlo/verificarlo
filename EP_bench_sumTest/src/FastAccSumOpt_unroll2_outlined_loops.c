...
double FastAccSumOpt_unroll2_outlined_loops_(double *p, unsigned int n) {
  double *q, r;
  
  q = (double *)malloc(sizeof(double)*n);
  memcpy(q, p, sizeof(double)*n);
  r = FastAccSumOpt_unroll2_outlined_loops_In(q, n);
  free(q);
  return r;
}

void faccU2_inner_loop(double *p, unsigned int n,double * sigma_ptr, double *sigma_prime_ptr) {
	double q;
	int i;
...
double FastAccSumOpt_unroll2_outlined_loops_In(double *p, unsigned int n) {
	...
	do
	{
		...
		faccU2_inner_loop(p, n,&sigma, &sigma_prime);
		...
		t_prime=t+tau;
		if (t_prime==0) 
			return res=FastAccSumOpt_unroll2_outlined_loops_In(p,n);
